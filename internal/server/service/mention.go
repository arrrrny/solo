...
// hereMention is the broadcast keyword: @here notifies every active agent
// member of the channel instead of a single named agent.
...
// agents that are active members of the channel. The broadcast keyword
// @here resolves to all active agent members of the channel.
func (s *MentionService) ResolveMentions(ctx context.Context, content, channelID string) (mentionedIDs []string, hasMentions bool, err error) {
	...
	// Deduplicate mention names, extracting the @here broadcast keyword.
	// @here resolves to every active agent member of the channel.
	...
	for _, m := range matches {
		...
		if strings.EqualFold(name, hereMention) {
			notifyAll = true
			continue
		}
		nameSet[name] = true
